...
lineas=$(($lineas-1))
existe=$(cat /etc/passwd | grep -i "$nombreusu" | wc -l)
...
for i in ${noexisten[@]}; do
	...
	if [ $exisgrupo -eq 0 ]; then
		...
		echo "El usuario $i se insertó el día $(date) y además se creó su grupo $grupo" >> nuevos_usuarios.log

	else

		sudo useradd -m -d $carper -s $interp -g $grupo $i 2>> nuevos_usuarios.log

		echo "El usuario $i se insertó el día $(date) a un grupo ya creado llamado $grupo" >> nuevos_usuarios.log
	fi
done
